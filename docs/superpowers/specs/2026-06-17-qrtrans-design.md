...
| `--grid` | `3x1` | array 网格 `RxC`；`auto` 表示按 `--screen`/`--module-px` 自动算最密布局 |
...
- 标准要求每 QR 四周 4 模块静默区。**阵列中相邻 QR 共享静默区**（中间一道间隔同时服务两侧）。
- 因此 R×C 阵列的总尺寸（模块）：
  - 宽 = `C×177 + (C+1)×4 = 181C + 4`
  - 高 = `R×177 + (R+1)×4 = 181R + 4`
- 像素尺寸 = 模块数 × `--module-px p`，再加可选的 40px 标签横幅（仅高度方向）。

### 6.2 1920×1080 下的可行布局（V40、含共享静默区、含 40px 横幅）

| `--module-px` | `--grid` | 帧像素（宽×高） | QR/帧 | 可扫性 |
|---|---|---|---|---|
| **3（默认）** | **3x1（默认）** | **1641×595** | **3** | **稳，截屏首选** |
| 3 | 2x1 | 1098×595 | 2 | 最稳 |
| 2 | 5x2 | 1818×772 | 10 | 密度最高，2px 模块在干净截屏下可扫但略冒险 |
| 4 | 2x1 | 1464×780 | 2 | 最稳，模块大 |

注：2×2 at 3px = 1098×1098，**超出 1080**，不可用；要 2×2 必须 V30 或更小模块。

### 6.3 默认值
- `--module-px 3`、`--grid 3x1` → 每帧 **3 个 QR**，帧尺寸 **1641×595 像素**，1920×1080 屏幕下两侧各留约 140px、下方留约 485px 余量，适配截屏。

### 6.4 自动布局（`--grid auto`）
给定 `--screen WxH`、`--module-px p`、横幅高 `B=40`：
- 最大列数 `C_max = floor((W / p - 4) / 181)`
- 最大行数 `R_max = floor(((H - B) / p - 4) / 181)`
- 取 `C_max × R_max`。

验算 1920×1080、p=3：`C_max = floor((640-4)/181) = 3`，`R_max = floor((1040/3 - 4)/181) = 1` → **3×1**，与默认一致。
p=2：`C_max = 5, R_max = 2` → **5×2 = 10 QR/帧**（激进模式）。
...
### 7.3 输出形态判定（无歧义规则）
按"结果集 + 输入类型"决定，不依赖 `-o` 末尾字符：

| 输入是目录？ | 解码出的文件数 | 目录标记数 | 行为 |
|---|---|---|---|
| 是 | 任意 | 任意 | `-o` 视为**目录**，按 `path` 重建层级 |
| 否（单张图） | 1 | 0 | `-o` 视为**文件路径**，直接写入 |
| 否（单张图） | 0 | 1 | `-o` 视为**目录**，创建空目录 |
| 否（单张图） | ≠1 文件 或 >0 目录 | | `-o` 视为**目录**，重建层级 |

歧义场景（如 `-o` 指向已存在的目录但结果按单文件）：报错并提示用户显式指定目录路径。
