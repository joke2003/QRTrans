...
## 全屏播放器（qrtrans-viewer）

真全屏 1:1 显示帧（用满每个像素、最大化密度）、目录内键盘切换、按间隔自动切换——针对 colormatrix/QR 帧的"显示→对端拍摄/截图"工作流。便携、独立 exe、不弹控制台。

### 下载

Release 里的 `qrtrans-viewer.exe`（与主 exe 同 release）。

### 与 encoder 协同（密度最大化）

**首次在该目录开一次 viewer**（哪怕几秒），它会把自己实测的全屏分辨率写入当前目录的 `qrtrans.json`；之后 encoder 在**未显式传 `--screen`** 时自动读它、按该尺寸出图 → 1:1 铺满、零浪费。无 config 时 encoder 默认 1080p；显式 `--screen` 永远最优先。

> 前提：viewer 与 encoder 在**同一目录**运行（encoder 读的是当前目录下的 `qrtrans.json`）。

### 用法

```
qrtrans-viewer <目录|图片> [--interval 3.0] [--loop] [--no-overlay]
```
...
- `--interval`：自动切换间隔，默认 3s，可小数。
- 角标（可用 `--no-overlay` 关闭）显示：`文件名 · i/N · 间隔 · ▶/⏸`。

### 注意

- 1:1 **不缩放**，故编码时 `--screen` 应 ≤ 实际屏（否则边缘被裁）。
- 首帧会显示 3 秒的分辨率提示（本机分辨率 + 建议 `--screen`）。
